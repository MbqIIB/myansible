...
from nova.gpus_kvm import GpuData
...
class ResourceTracker(object):
    ...
    def _update_available_resource(self, context, resources):
        ...
        # Add by jian for debug
        gpu_data = GpuData()
        gpu_data.open_db()
        result = gpu_data.get_available_gpu()
        gpu_data.close_db()
        LOG.info("jian... get nvidia data from DB:%s" % result)
        self.compute_node.extra_resources = len(result)

        self._report_final_resource_view()

        #LOG.info("jian get the compute_node:%s" % self.compute_node)
        LOG.info("jian... compute_node name:%s, avaliable GPU count:%s" % (self.compute_node.hypervisor_hostname, self.compute_node.extra_resources))
    ...
    def _report_final_resource_view(self):
        ...
        LOG.info(_LI("Final resource view: "
                 ...
                 {'node': self.nodename,
                  ...
                  'total_vcpus': tcpu,
                  'used_vcpus': ucpu,
                  #'total_gpus': self.compute_node.total_gpus,
                  'pci_stats': pci_stats})
